(function () {
  ...
  function convertGltfMaterials(root) {
    ...
    root.traverse(function (o) {
      if (o.isMesh && o.material) {
        ...
        if (o.isSkinnedMesh) m2.skinning = true;   // r128 Lambert needs the flag or bones do nothing
        if (m2.map) {
  ...
  // yard is never full of T-posing zombies.
  var MIXERS = [];
  var ANIM_CLIPS = {};
  var ROLE_CLIP = { babushka: "old_idle", landlord: "idle", bigguy: "idle", kid: "walk",
                    man_a: "idle", man_b: "idle", woman_a: "idle", woman_b: "idle" };
  function findHips(root) {
    var h = null;
    root.traverse(function (o) { if (!h && /Hips$/.test(o.name)) h = o; });
    return h;
  }
  // Box3.setFromObject reads raw bind-space vertices on a SkinnedMesh (useless for
  // sizing a rigged model) — run each vertex through the skin at rest pose instead.
  function skinnedBox(inst) {
    inst.updateMatrixWorld(true);
    var box = new THREE.Box3(), v = new THREE.Vector3(), any = false;
  ...
  deli.userData.clip = "talk";   // mid-lecture about the pelmenai, permanently
  ...
  tour1.userData.clip = "talk";   // the couple argue about the map forever
  ...
  // ---------- gym / garden first-person arms ----------
  var gymRig = new THREE.Group(); camera.add(gymRig); gymRig.visible = false;
  var gSkin = C(0xdcab7c), gFlesh = C(0xd49a6a);
  var gArmL = new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.09, 0.42), gSkin); gymRig.add(gArmL);
  ...
  [[0xc9423a, 0x2c4a6e, 0], [0x3ac96a, 0x4a3550, Math.PI]].forEach(function (kf) {
    ...
    kd.userData.clipRate = 1.6;   // kids scurry
    gS.add(kd);
  ...
  dj.userData.clip = "talk";   // reads as MC-ing the booth
  ...
  [[0x4a3550, 0x16161c], [0x2c4a6e, 0x101014], [0x6e3a3a, 0x16161c]].forEach(function (dc, i) {
    ...
    d.userData.clip = ["dance_a", "dance_b", "drunk_idle"][i];   // third guy is past dancing
    d.position.set(NX + 4.5 + i * 2.2, 0, 4.5 + (i % 2)); gE.add(d);
    dancers.push(d);
  });
  [[0xe07aa8, 0x16161c, 0xe8d36b, NX + 5.5, 6.2], [0xc92c4a, 0x101014, 0x4a3015, NX + 9, 5.2]].forEach(function (gf, gi) {
    var g = makePerson("woman_a", gf[0], gf[1], null, 0.92, gf[2]);
    g.userData.clip = gi ? "dance_b" : "dance_a";
  ...
  var TCX = -400, TCZ = 0;
  var T_AXO = 60, T_AZO = 38, T_AXI = 40, T_AZI = 22; // outer/inner semi-axes
  box(gT, grassM, TCX - 82, -0.06, TCZ - 78, TCX + 82, -0.02, TCZ + 62);
  (function () {
    var sh = new THREE.Shape();
    sh.absellipse(0, 0, T_AXO, T_AZO, 0, Math.PI * 2, false, 0);
    var ho = new THREE.Path();
    ho.absellipse(0, 0, T_AXI, T_AZI, 0, Math.PI * 2, true, 0);
    sh.holes.push(ho);
    var m = new THREE.Mesh(new THREE.ShapeGeometry(sh, 64), roadM);
    m.rotation.x = -Math.PI / 2; m.position.set(TCX, 0.01, TCZ); gT.add(m);
    var sh2 = new THREE.Shape();
    sh2.absellipse(0, 0, T_AXI - 0.4, T_AZI - 0.4, 0, Math.PI * 2, false, 0);
    var m2 = new THREE.Mesh(new THREE.ShapeGeometry(sh2, 48), grassM);
    m2.rotation.x = -Math.PI / 2; m2.position.set(TCX, 0.02, TCZ); gT.add(m2);
  })();
  // start/finish checker across the bottom straight (z = TCZ - T_AZO .. -T_AZI)
  var segL = (T_AZO - T_AZI) / 8;
  for (var sf = 0; sf < 8; sf++) for (var sg = 0; sg < 2; sg++) {
    box(gT, ((sf + sg) % 2 === 0) ? whiteM : darkM,
      TCX - 0.8 + sg * 0.8, 0.03, TCZ - T_AZO + sf * segL,
      TCX + sg * 0.8 - 0.8 + 0.8, 0.05, TCZ - T_AZO + (sf + 1) * segL);
  }
  // pit / exit stub running south from the bottom straight
  box(gT, roadM, TCX - 6, 0.0, TCZ - 64, TCX + 2, 0.03, TCZ - 30);
  // start gantry
  cyl(gT, greyM, 0.2, 0.2, 6, 3, 6).position.set(TCX - 8, 3, TCZ - 30);
  cyl(gT, greyM, 0.2, 0.2, 6, 3, 6).position.set(TCX + 8, 3, TCZ - 30);
  box(gT, greyM, TCX - 8, 5.9, TCZ - 30.1, TCX + 8, 6.1, TCZ - 29.9);
  box(gT, B(textTex(256, 48, "#1a1a1a", "#f0ece0", ["START / FINISH"], 18)), TCX - 8, 5.2, TCZ - 30.15, TCX + 8, 6.0, TCZ - 30.05);
  // TRASA / exit sign at the pit mouth
  plane(gT, B(textTex(256, 64, "#1c6e3a", "#f0f4f8", ["TRASA", "← A1 isvaziavimas"], 18)), 8, 2.2, TCX + 7, 2.6, TCZ - 58, -Math.PI / 2);
  // grandstand on the far side
  box(gT, greyM, TCX - 26, 0, TCZ + T_AZO + 4, TCX + 26, 5.5, TCZ + T_AZO + 10);
  // perimeter trees
  [[-72, -22], [-72, 30], [72, -22], [72, 30], [0, 56], [-44, -60], [44, -60], [-30, 52], [30, 52]].forEach(function (tp) {
    var tr = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.3, 2.6, 6), woodM);
    tr.position.set(TCX + tp[0], 1.3, TCZ + tp[1]); gT.add(tr);
    var cn = new THREE.Mesh(new THREE.ConeGeometry(1.9, 4.2, 7), greenM);
    cn.position.set(TCX + tp[0], 4.6, TCZ + tp[1]); gT.add(cn);
  });
  ...
  var radioOn = false, radI = 0, nbrMet = false;
  var smkT = 0, smkDragged = [false, false, false];
  var gardenDone = false, senI = 0, dayCount = 1, v6Intro = false, driveIntro = false;
  var rentIntroDone = false, rentDueDay = 7, landlordKnock = false, lastKnock = -99, rentMiss = 0;
  var LORDW = "SEIMININKAS";
  ...
  function doTaromat() {
    ...
    AU.taromat();
    for (var i = 0; i < Math.min(empties, 3); i++) AU.beep(880 - i * 40, 0.08, "sine", 0.02);
  ...
  // ---------- choice menu ----------
  var choiceEl = $("choice"), choiceTitle = $("choicetitle"), choiceBtns = $("choicebtns");
  function closeChoice() { choiceEl.style.display = "none"; if (mode === "choice") mode = "walk"; }
  function choose(title, opts) {
    mode = "choice"; unlockPtr();
    choiceTitle.textContent = title;
    choiceBtns.innerHTML = "";
    opts.forEach(function (o) {
      var b = document.createElement("button");
      b.textContent = o.l;
      b.onclick = function (e) { e.stopPropagation(); choiceEl.style.display = "none"; mode = "walk"; o.f(); };
      choiceBtns.appendChild(b);
    });
    choiceEl.style.display = "flex";
  }
  $("choicex").onclick = function (e) { e.stopPropagation(); closeChoice(); };
  choiceEl.addEventListener("pointerdown", function (e) { e.stopPropagation(); });
  ...
  function doGarden() {
    ...
    say([{ w: "SENELE", t: "Geras berniukas. Gloves are by the well. Mind senelis's currant bushes." },
      { t: "You roll up your sleeves and crouch into the first row." }], function () {
      repPay = 8;
  ...
  // ---------- rent / the landlord ----------
  function newDayLandlord() {
    if (!rentIntroDone) landlordKnock = true;
    else if (dayCount >= rentDueDay) landlordKnock = true;
  }
  function landlordEnter() {
    landlordKnock = false; lord.visible = true;
    // frame the scene: stand the player back from the door facing it, and put
    // Vytautas in the open doorway facing back — otherwise he spawns on top of
    // the camera (you answer the door from ~x7, where he used to stand)
    pos.set(6.3, 0, 2.7); baseY = FY; yaw = -Math.PI / 2; pitch = 0;
    lord.position.set(7.55, FY, 2.7); lord.rotation.y = Math.PI / 2;
    var done = function () { lord.visible = false; saveGame(); };
    if (!rentIntroDone) {
      rentIntroDone = true; rentDueDay = dayCount + 7;
      say([
        { w: LORDW, t: "Labas. I am Vytautas. Your landlord. This flat is mine. You are just the noise inside it." },
        { w: LORDW, t: "Twenty euro. Every week. I come back in seven days. Have it ready, or we have a problem." },
        { t: "He looks around your flat once, decides it confirms every suspicion he arrived with, and leaves. Rent: 20 EUR, due in 7 days." }
      ], done);
    } else if (money >= 20) {
  ...
  var ZONES = {
    ...
    track: { n: "TRASA — the lap circuit", short: "TRASA", min: 16,
      entry: { x: TCX - 2, z: TCZ - 56, yaw: Math.PI },
      fl: ["no cameras out here, allegedly", "the tyres warm up on the way over"] }
  };
  function showTravel() {
  ...
  function exitShop() { AU.door(); toYard(5, 42.6, 0); }
  function toGym() {
    if (!gymOpen()) {
  ...
  function exitGym() { AU.door(); toYard(39, 42.6, 0); }
  function doClub() {
    if (!isNight()) {
  ...
  // ---------- radio wheel ----------
  var rwheelEl = $("rwheel"), rwnowEl = $("rwnow"), rwOn = false;
  AU.onRadio = function (t) { showCap(t); rwUpd(); };
  function rwUpd() {
    var inf = AU.radioInfo();
    rwnowEl.textContent = inf.on ? inf.name : "TYLA / OFF";
    [["rwPopas", "popas"], ["rwBlokas", "blokas"], ["rwKlaip", "klaip"]].forEach(function (p) {
      $(p[0]).classList.toggle("sel", inf.on && inf.st === p[1]);
    });
    $("rwOff").classList.toggle("sel", !inf.on);
  }
  function rwOpen() {
    if (mode !== "drive") return;
    rwOn = true; rwUpd(); rwheelEl.style.display = "flex"; unlockPtr();
  }
  function rwClose() { rwOn = false; rwheelEl.style.display = "none"; }
  function rwPick(st) {
    radioOn = !!st;
    AU.radioTune(st);
    if (!st) showCap("radio off. just the V6 and your thoughts. mostly the V6.");
    rwClose();
  }
  $("rwPopas").onclick = function () { rwPick("popas"); };
  $("rwBlokas").onclick = function () { rwPick("blokas"); };
  $("rwKlaip").onclick = function () { rwPick("klaip"); };
  $("rwOff").onclick = function () { rwPick(null); };
  rwheelEl.addEventListener("pointerdown", function (e) {
    e.stopPropagation();
    if (e.target === rwheelEl) rwClose();
  });
  function radioCap() {
    var inf = AU.radioInfo();
    if (inf.on) showCap("\u266a " + inf.name + " \u00b7 [R] radio wheel");
  }
  function startDrive() {
    ...
    if (!driveIntro) {
      driveIntro = true;
      say([{ t: "Mirror. Signal. The IID hums on the dash like a parole officer. Drive carefully — half the yard is watching. [R] radio · [H] horn" }]);
  ...
  function items() {
    var arr = [
      ...
      { ar: "flat", x: 7.9, z: 2.7, r: 1.5, l: landlordKnock ? "Answer the door — somebody's hammering" : "Leave the flat", f: function () { if (landlordKnock) landlordEnter(); else toHall(); } },
  ...
  window.addEventListener("keydown", function (e) {
    ...
    if (e.code === "KeyR" && mode === "drive" && !e.repeat) {
      if (rwOn) rwClose(); else rwOpen();
    }
    if (e.code === "KeyH" && mode === "drive" && !e.repeat) AU.horn();
    if (rwOn && !e.repeat) {
      if (e.code === "Digit1") rwPick("popas");
      else if (e.code === "Digit2") rwPick("blokas");
      else if (e.code === "Digit3") rwPick("klaip");
      else if (e.code === "Digit0") rwPick(null);
      else if (e.code === "Escape") rwClose();
    }
  ...
  function loop() {
    ...
    // the landlord at the door — loud banging until you answer
    if (landlordKnock && (area === "flat" || area === "hall") && mode === "walk" && et - lastKnock > 3.6) {
      lastKnock = et;
      AU.door(); setTimeout(AU.door, 260); setTimeout(AU.door, 520);
      showCap(area === "flat"
        ? "BANG BANG BANG — someone is hammering on your door. (answer it)"
    ...
    AU.setEnv(area === "yard" || area === "old" || area === "track" || mode === "drive" ? "out" :
    ...
    girls.forEach(function (g) {
      if (g.fixed) {
        if (!g.m.userData.anim) g.m.position.y = Math.abs(Math.sin(et * 4.3 + g.m.position.x)) * 0.14;
        return;
      }
      g.x += g.dir * g.sp * dt;
    ...
    if (gE.visible) {
      ...
      dancers.forEach(function (d2, i) {
        if (d2.userData.anim) return;   // the dance clip does the moving
    ...
    if (mode === "wake") {
      ...
      spd += thr * 9 * dt; spd -= spd * 0.6 * dt;
      spd = Math.max(-4.5, Math.min(15, spd));
      carYaw += st * 1.5 * dt * Math.min(1, Math.abs(spd) / 4) * (spd < 0 ? -1 : 1);
      if (Math.abs(st) > 0.5 && Math.abs(spd) > 9) AU.skid();
      if (thr < -0.4 && spd > 7) AU.brake();
      var cfx = -Math.sin(carYaw), cfz = -Math.cos(carYaw);
      var nx2 = car.position.x + cfx * spd * dt, nz2 = car.position.z + cfz * spd * dt;
      var hit = false;
      if (area === "track") {
        var dxo = nx2 - TCX, dzo = nz2 - TCZ;
        var no = (dxo * dxo) / (T_AXO * T_AXO) + (dzo * dzo) / (T_AZO * T_AZO);
        var ni = (dxo * dxo) / (T_AXI * T_AXI) + (dzo * dzo) / (T_AZI * T_AZI);
        var inPit = nx2 > TCX - 6 && nx2 < TCX + 2 && nz2 > TCZ - 64 && nz2 < TCZ - 28;
        if (!inPit && (no > 1.0 || ni < 1.0)) hit = true;
      } else {
      ...
      if (hit) {
        if (Math.abs(spd) > 6) AU.skid();
        spd *= -0.25; AU.beep(120, 0.12, "square", 0.06);
      }
      else { car.position.x = nx2; car.position.z = nz2; }
      if (area === "track") {
        var rrx = car.position.x - TCX, rdz = car.position.z - TCZ;
        if (rdz > 15) lapArmed = true;
        if (lapArmed && Math.abs(rrx) < 6 && rdz < -16 &&
            ((lapPrevRX < 0 && rrx >= 0) || (lapPrevRX > 0 && rrx <= 0))) {
          lapN++; var lt = et - lapStartT; lapStartT = et; lapArmed = false;
          var mm = Math.floor(lt / 60), ss = lt % 60;
          showCap("LAP " + lapN + " — " + mm + ":" + (ss < 10 ? "0" : "") + ss.toFixed(1));
          AU.beep(990, 0.12, "sine", 0.06);
        }
        lapPrevRX = rrx;
      }
      ...
      AU.engineSet(spd);
      // driver's eye: raised and pushed forward toward the windscreen, tilted
      // down a touch so the long bonnet doesn't eat the road ahead
      var lxo = -0.34, lzo = -0.55;
      camera.position.set(
        car.position.x + lxo * Math.cos(carYaw) + lzo * Math.sin(carYaw), 1.32,
        car.position.z - lxo * Math.sin(carYaw) + lzo * Math.cos(carYaw));
      camera.rotation.set(-0.11, carYaw + lookOff, Math.sin(et * 1.6) * 0.015 * bac);
      ...
      if (mode === "walk") {
        ...
        var opx = pos.x, opz = pos.z;
        slide((fx * iz + rx * ix) * sp * dt, (fz * iz + rz * ix) * sp * dt, curCols());
        stepAcc += Math.sqrt((pos.x - opx) * (pos.x - opx) + (pos.z - opz) * (pos.z - opz));
        if (stepAcc > 0.62) {
          stepAcc = 0;
          AU.step(area === "hall" ? "stair" :
            area === "pond" ? "grass" :
            (area === "yard" || area === "old" || area === "track" || (area === "flat" && pos.x < 0)) ? "concrete" :
        ...
        if (area === "track") {
          pos.x = Math.max(TCX - T_AXO - 2, Math.min(TCX + T_AXO + 2, pos.x));
          pos.z = Math.max(TCZ - 66, Math.min(TCZ + T_AZO + 2, pos.z));
        }
      }
      if (mode === "smoke") {
